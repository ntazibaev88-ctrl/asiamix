...
export default function AdminFinancePage() {
  const { t } = useI18n();
  const gmv = demoStores.reduce((s, x) => s + x.revenue, 0);
  const commission = demoStores.reduce(
    (s, x) => s + (x.revenue * x.commission) / 100,
    0,
  );
    ...
    <>
      ...
      <div className="grid grid-cols-2 gap-4 lg:grid-cols-4">
        ...
        <StatCard
          icon={Banknote}
          label="Выплаты"
          value={formatPrice(420000)}
        />
        <StatCard
          icon={Wallet}
          label="Cashback"
          value={formatPrice(86000)}
        />
      </div>
        ...
        <div className="flex flex-col gap-3">
          {demoStores.map((s) => (
            <div key={s.name} className="flex items-center gap-3">
              ...
              <span className="text-xs text-faint">{s.commission}%</span>
              <span className="w-28 text-right text-sm font-semibold">
                {formatPrice(Math.round((s.revenue * s.commission) / 100))}
              </span>
            </div>
          ))}
        </div>
